...
#### 4.103.0 - (Unreleased)
* **PERFORMANCE**: `ArrayConversions.arrayToArray` and `collectionToArray` defer the `IdentityHashMap` allocation used for cycle detection until the first nested array/collection is actually encountered, and skip cycle tracking entirely when the target component type is primitive (no cycles possible with scalar elements). Previous behavior unconditionally allocated the map and put the root mapping at function entry — wasted work for the common flat-array / flat-collection case. Profile screenshots showed ~130 ms for the `new IdentityHashMap<>()` plus ~430 ms for the root `put` accumulated across many calls; both are eliminated for primitive-target conversions and deferred to first-need for reference-target conversions. Behavioral semantics preserved: when nested arrays/collections do appear, the map is allocated with size 16 and seeded with the root mapping on first encounter, then operates identically to before.
