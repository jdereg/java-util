...
#### 4.103.0 - (Unreleased)
* **FEATURE / PERFORMANCE**: New `com.cedarsoftware.util.internal.VectorizedArrays` helper exposes range-based array operations (`equalsRange`, `mismatchRange`, `compareRange` — each for both `char[]` and `byte[]`) that dispatch to JDK 9+ `Arrays.*(arr, int, int, arr, int, int)` intrinsics (SIMD-vectorized on supported HW) at runtime, with hand-rolled loop fallbacks for JDK 8. Bridges the gap between java-util's JDK 8 source/target floor and the modern JVMs the library actually runs on in production. Dispatch mechanics: one-time resolution at class load via `SystemUtilities.isJavaVersionAtLeast(9, 0)` + `MethodHandles.publicLookup().findStatic(...)`, cached in `static final MethodHandle` fields (HotSpot inlines `invokeExact` on these). Per-call cost is one static-field read + null-check + (on JDK 9+) `MH.invokeExact`. No per-call version detection. Visibility is internal package (JPMS-exported to `com.cedarsoftware.io`); promote to public API once contract stabilizes. `System.arraycopy` deliberately omitted — already a HotSpot intrinsic on JDK 8+, no slower fallback to dispatch to. 21 new tests in `VectorizedArraysTest`.
* **PERFORMANCE**: Cycle-tracking `IdentityHashMap` allocation deferred to first-need across four iterative conversion methods: `ArrayConversions.arrayToArray` / `ArrayConversions.collectionToArray` (which also gain a "skip-for-primitive-target" fast-path since primitive arrays can't form cycles), and `CollectionConversions.arrayToCollection` / `CollectionConversions.collectionToCollection`. Previous behavior unconditionally allocated the map and put the root mapping at function entry — wasted work for the common flat-array / flat-collection case (the map is only consulted inside the nested-element branch). Profile screenshots showed ~130 ms for the `new IdentityHashMap<>()` plus ~430 ms for the root `put` accumulated across many calls in `ArrayConversions.arrayToArray` alone; both are eliminated for primitive-target conversions and deferred to first-need for reference-target conversions. Behavioral semantics preserved: when nested arrays/collections do appear, the map allocates with initial capacity 16 and is seeded with the root mapping on first encounter, then operates identically to before.
* **FEATURE**: `Converter.convert(String, byte[].class)` (and transitively `String → ByteBuffer`) now performs multi-format detection. Previous behavior was charset-encoding only (`s.getBytes(charset)`); the new path tries in order: (1) stringified JSON number array `"[1, 2, 3]"`, (2) spaced hex `"CA FE BA BE"` (≥ 2 pairs), (3) unspaced hex `"CAFEBABE"` (length ≥ 8 — catches Java magic numbers like `CAFEBABE`/`DEADBEEF`, compact 32-char UUIDs, SHA-1/SHA-256 hex hashes), (4) URL-safe Base64 (alphabet `[A-Za-z0-9_-]=`), (5) standard Base64 (alphabet `[A-Za-z0-9+/]=`). Both Base64 layers apply a "tight" rule (length ≥ 16 OR contains `=` padding OR contains `+/_-` symbol) so short text tokens like `"DATA"`, `"CODE"`, `"TEST"` continue to fall through to the historical charset path. Hex layers apply parallel tightness (unspaced ≥ 8 chars; spaced ≥ 2 pairs / 5 chars) to prevent short identifiers like `"ABCD"`, `"0123"`, `"ab"` from being mis-classified. Documented residual edge case: a 16+-character string in the standard Base64 alphabet containing `/` (e.g. `"prod/2026/backup"`) will Base64-decode under the length≥16 trigger — accepted as vanishingly rare for `byte[]` field usage. Real-world wins: Jackson-emitted Base64 byte arrays now round-trip transparently; JWT segments (URL-safe Base64) decode; common debug magic numbers like `CAFEBABE` decode correctly to 4 bytes via the hex layer; compact UUIDs decode to 16 bytes.
* **FEATURE**: New `MapConversions.toByteArray` converter (and refactored `MapConversions.toByteBuffer`) for the wrapped-form contract used by json-io's tree writer: `{"@type":"<type>","value":"<base64>"}` (or simply `{"value":"..."}` / `{"_v":"..."}`). When the extracted value is a String, attempts strict `Base64.getDecoder().decode(...)` first — this preserves round-trip for short blobs whose Base64 encoding is too short to pass the tight rules in the standalone `String → byte[]` path (e.g. a 3-byte input encodes to 4 unpadded chars like `"AQL9"`). On invalid Base64, falls through to `Converter.convert(...)` dispatch so other string formats (stringified JSON arrays, hex, etc.) still work in the Map context. Non-String values dispatch through Converter directly, supporting other source types (other ByteBuffer, byte[], CharBuffer, etc.). Wire-form `Map → byte[]` is the symmetric reverse of json-io's existing `byte[] → JsonObject` write path, enabling clean round-trip for the wrapped form that `JsonGenerator.writeBinary` will emit in json-io 4.103.0.
* **TESTING**: 80 new `ConverterEverythingTest` fixtures covering the format-detection ladder across `pair(String.class, byte[].class)`, `pair(String.class, ByteBuffer.class)`, `pair(Map.class, byte[].class)`, and the extended `pair(Map.class, ByteBuffer.class)`. Includes the documented charset-fallback cases (`DATA`/`CODE`/`TEST`/`ABCD`/`0123`/`ab`/`hello world`/`AAAA`) to pin the tight-rule boundary.
