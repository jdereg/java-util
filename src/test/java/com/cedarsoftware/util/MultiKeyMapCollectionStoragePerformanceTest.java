...
public class MultiKeyMapCollectionStoragePerformanceTest {
    ...
    private static final Logger LOG = Logger.getLogger(MultiKeyMapCollectionStoragePerformanceTest.class.getName());
    ...
    void testCurrentImplementation() {
        LOG.info("=== CURRENT IMPLEMENTATION (Converts non-RandomAccess to Object[]) ===");
    ...
    private void testPerformance(String description, MultiKeyMap<String> map, List<?> keys) {
        ...
        LOG.info(String.format("  Populate: %,d entries in %,d ms",
    ...
    private void testMixedScenario() {
        LOG.info("Mixed scenario (store LinkedList, lookup with ArrayList):");
        ...
        LOG.info(String.format("  Store with LinkedList: %,d ms",
        ...
        LOG.info(String.format("  Lookup with ArrayList: %,d hits in %,d ms",
            hits, TimeUnit.NANOSECONDS.toMillis(lookupTime)));
        LOG.info(String.format("  Cross-container lookup rate: %,.0f lookups/second",
