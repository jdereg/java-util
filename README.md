...
### Template Engine
n-cube can be used to return templates (think of them as HTML pages, for example, with replaceable parts - like mail merge).  When a template cell is executed, variables within the template are replaced (think mail merge).  If you have used the Apache project's Velocity project, Groovy templates, or have written JSP / ASP files, then you already have an idea on how to use templates.
