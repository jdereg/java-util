...
* **PERFORMANCE**: `DateUtilities` — eliminated regex-call hot-path overhead identified in JFR (~2.7s self-time across regex bookkeeping during a `JsonPerformanceTest` run). Two changes: (1) **Cached security configuration**. The five hot getters (`isSecurityEnabled`, `isInputValidationEnabled`, `isMalformedStringProtectionEnabled`, `isRegexTimeoutProtectionEnabled`, `getRegexTimeoutMilliseconds`) previously called `System.getProperty(...)` per parse — each grabbed the synchronized `Hashtable` lock on system properties. Replaced with `volatile` tri-state (`UNRESOLVED`/`FALSE`/`TRUE`) caches; cost drops to a `volatile int` load + branch. New package-private `resetSecurityConfigCacheForTesting()` invalidates the cache after `System.setProperty(...)` calls. (2) **Per-`Pattern` thread-local `Matcher` reuse**. `safeMatches`/`safeFind` previously called `pattern.matcher(input)` per call, allocating a `Matcher` plus its `groups[]`/`locals[]`/`localsPos[]` arrays. Replaced with a `ConcurrentMap<Pattern, ThreadLocal<Matcher>>` cache: each thread keeps one reusable `Matcher` per `Pattern`, reset to new input each call. Slow paths (timeout-protected via `DATE_REGEX_EXECUTOR`) still allocate fresh — they cross thread boundaries and can't safely share a thread-local. `SafeMatchResult` snapshot semantics preserved (no `Matcher` reference held after construction).
* **PERFORMANCE**: `MathUtilities` — added 20 fast number-parsing entry points: `parseDouble`, `parseFloat`, `parseBigDecimal`, `parseBigInteger`, each with `CharSequence`, `char[]`, `char[]+offset+length`, `byte[]`, and `byte[]+offset+length` overloads. The `char[]` and `byte[]` overloads avoid the intermediate `String` materialization that callers like JSON parsers previously paid before `Double.parseDouble`. Backed by the vendored Randelshofer FastDoubleParser; output bit-identical to JDK reference parsers (150-test `FastDoubleParserParityTest` covers NaN, ±Inf, denormals, hex, scientific, 100-/500-digit BigIntegers, malformed-input parity). Converter rewiring: `StringConversions.toFloat`/`toDouble`/`toBigDecimal`/`toBigInteger`/`toLong`/`toDuration`, `NumberConversions.floatingPointToBigInteger`, and `MathUtilities.parseToMinimalNumericType` route through these entry points. Anything calling `Converter.convert(String, Double.class)` or analogous picks up the speedup transparently.
* **PERFORMANCE**: Vendored Werner Randelshofer's FastDoubleParser (MIT licensed, https://github.com/wrandelshofer/FastDoubleParser, Java 8 source set) under `com.cedarsoftware.util.fastdoubleparser`. Eisel-Lemire (2020) algorithm for `Double`/`Float`; recursive-multiplication parsers for `BigDecimal`/`BigInteger`. 2-4x faster than `Double.parseDouble` on typical inputs; 5-50x faster than `new BigInteger(String)` on values with 100+ digits. Mirrors Jackson's vendoring choice (jackson-core 2.15+) — keeps java-util dependency-free. Pruned the locale-aware `Configurable*` parsers and JSON-strict `Json*` variants since the universal `JavaDoubleParser`/`JavaFloatParser` cover every shape we need. Source set: 34 files (~5K LOC). Jar size 736 KB → 849 KB (+15.4%).
* **PERFORMANCE**: `FastReader` — added borrowed-slice fast paths `readUntilBorrowed(BufferSlice, int, char, char)` and `readLineBorrowed(BufferSlice)` for callers that can consume directly from the internal buffer. Existing delimiter semantics preserved (`readUntil` leaves the delimiter unread; `readLine` consumes the line ending). Returns `COPY_REQUIRED` without consuming data when pushback is active or the token crosses a buffer boundary, so callers fall back cleanly to the copying APIs. JSON/TOON consumers avoid the buffer-to-scratch copy on common short-string/whole-line cases — JFR shows the old copy paths effectively disappear: `readUntil(...)` 101→1 (toJava) and 76→4 (toMaps); `readLine(...)` 83→0 and 78→0.

#### 4.101.0 - 2026-04-19
* **BUG FIX**: `ReflectionUtils.getAllConstructorsInternal()` — added a deterministic tie-breaker to the constructor sort. `Class.getDeclaredConstructors()` returns constructors in JVM-dependent order that varies across runs (especially under bytecode instrumentation like JaCoCo, AspectJ). When modifier/param-count ties occurred (e.g., two public single-arg varargs constructors `foo(String...)` vs `foo(Integer...)`), stable sort preserved non-deterministic input order. Fix: tie-breaker prefers `String` parameters (most widely convertible via `Converter`), unwrapping varargs component types, then falls back to alphabetical type-name. Resolves intermittent ~30% failure rate in `ClassUtilitiesVarargsArrayStoreTest` under JaCoCo.
* **BUILD**: Added JaCoCo code coverage plugin (`jacoco-maven-plugin` 0.8.12) permanently. `prepare-agent` and `report` goals wired in; surefire `argLine` updated to `@{argLine}` for agent injection. Coverage reports generated automatically on every `mvn test` at `target/site/jacoco/`. No impact on Maven Central deployment or build artifacts.
* **CORRECTNESS**: `ClassValueMap` — marked the lazily-populated `cachedEntrySet`, `cachedValues`, and `cachedUnmodifiableView` fields `volatile`. Without volatile, a thread reading via the non-synchronized fast path could observe a partially-constructed view (Java allows stores to be reordered with the publishing store). The race is narrow and unobserved in practice but the fix eliminates a dormant JMM bug. No behavior change, no perf impact.
* **FEATURE**: `ClassValueSet.containsClass(Class)` — mirrors `ClassValueMap.getByClass`. Skips the null check and `o.getClass() != Class.class` guard that `contains(Object)` must perform to reject non-`Class` keys; compiles to a near-direct `ClassValue.get(type)` call. Converted 9 hot-path call sites: `MultiKeyMap` (5 — `LEAF_TYPES` ×3, `SIMPLE_ARRAY_TYPES`, `INTEGRAL_TYPES`) and `ClassUtilities.SECURITY_CHECK_CACHE` (4 — `BLOCKED_CLASSES` ×2, `INHERITS_FROM_BLOCKED`, `VERIFIED_SAFE_CLASSES`). Tightened `MultiKeyMap.SIMPLE_ARRAY_TYPES` from `Set<Class<?>>` to `ClassValueSet`. 6 new tests in `ClassValueSetTest`.
* **FEATURE**: `ClassValueMap.getByClass(Class)` — typed fast-path lookup that skips the `instanceof Class` guard in `get(Object)`. The regular `get(Object)` must check the key type before routing to the `ClassValue` cache (non-`Class` keys fall through to the `ConcurrentHashMap` backing). `getByClass(Class)` skips that guard, compiling to a near-direct `ClassValue.get(type)` — a JIT-intrinsified per-`Class` load. Converted 30+ hot-path call sites across `ClassUtilities` (13), `Converter.ClassPairMap` (4), `MultiKeyMap` (3), `TypeUtilities`, `Unsafe`, `CompactMap`, `CollectionHandling`. Field declarations storing a `ClassValueMap` behind `Map<Class<?>, V>` were tightened to `ClassValueMap<V>` so the typed lookup resolves statically. 7 new unit tests in `ClassValueMapTest`.
* **FEATURE**: `ReflectionUtils.getDirectClassAnnotation(Class, Class)` — strict, JDK-semantics companion to the existing `getClassAnnotation`. The existing method does a Spring-`AnnotationUtils.findAnnotation`-style full-hierarchy walk (class + all superclasses + interfaces, regardless of `@Inherited`), which is right for framework scanning but wrong for identity-tied annotations like JPA `@Entity`, json-io `@IoNaming`/`@IoTypeName`, or Jackson `@JsonNaming` where the author deliberately chose not to mark `@Inherited`. The new method matches `Class.getAnnotation(Class)` exactly — respects `@Inherited` on the superclass chain, never walks interfaces. Deliberately bypasses `CLASS_ANNOTATION_CACHE` because `Class.reflectionData()` already caches annotation proxies internally. Javadoc on both methods now calls out the divergent semantics. 13 new tests in `ReflectionUtilsDirectAnnotationTest`. Purely additive.
* **PERFORMANCE**: `StringUtilities` — restored public `getChars(String, int)` and added `getChars(String)` overload, plus extended the SIMD-intrinsic bulk-copy pattern to three more hot paths: `levenshteinDistance`, `damerauLevenshteinDistance`, and `decode(String)` (hex). The two-arg form lets callers pass a precomputed length; the one-arg form computes `s.length()` once. `hashCodeIgnoreCase` already used the pattern (4.99.0 CaseInsensitiveMap perf win). Both Levenshtein variants bulk-copy inputs once into local `char[]` before the O(n·m) nested loop and hoist loop-invariant chars out of the inner loop — `damerauLevenshteinDistance` drops from up to 6 `charAt` calls per inner iter (3× source/target each for cost calc + transposition) to 0 charAt + plain array loads. `decode(String)` reads pair-wise nibbles from the bulk-copied buffer. New private `fillCharArray(CharSequence, char[], int)` helper uses `String.getChars` on Strings and falls back to `charAt` for other CharSequences (StringBuilder, CharBuffer, etc.). CaseInsensitiveMap perf tests confirm no regression. Break-even ~7 chars; applied call sites comfortably exceed it.
* **PERFORMANCE**: `FastReader.readUntil` / `readLine` — eliminated redundant `GETFIELD` instructions by caching member fields into locals where the local is read 2+ times within its lifetime (per bytecode-level review). Specific wins: `pushbackBufferSize`/`pushbackBuffer` hoisted into `pbSize`/`pbBuf` for the drain loop; drain wrapped in `if (pbPos < pbSize)` to skip the wasteful unconditional `pushbackPosition` `PUTFIELD` in the empty-pushback common case; `buf`/`position`/`limit` hoisted out of the outer scan-while (`buf` never reassigned by `refill()`); `readUntil`'s in-loop refill moved to the **top** of the scan loop (kills the unconditional fill that fired even when scan hit `maxLen` instead of buffer-end); redundant `position = pos` writes removed. Hot-path field-access count in `readUntil` common case: **10 → 7 (-30%)**. Same reduction in `readLine`'s common case; multi-chunk lines save 3 GETFIELDs per additional fill.
* **PERFORMANCE**: `FastReader.readLine(char[], int, int)` — hoisted `b[scanPos]` into a single explicit local per scan-loop iteration instead of relying on JIT CSE to coalesce two array loads. Bytecode verification confirmed only one `CALOAD` (and one bounds check) in the emitted hot-path body after the refactor. JFR: `readLine` leaf samples 208 (2.09%) → 170 (1.65%), a **~22% relative reduction** in self-time, ~1s wall-clock saved on a 200s `JsonPerformanceTest` run. Outer `c <= '\r'` performance prefilter retained as a nested `if` so printable characters still need only one comparison before increment.
* **PERFORMANCE**: `Converter` — refactored the four internal conversion caches (`CONVERSION_DB`, `FULL_CONVERSION_CACHE`, `USER_DB`, `cacheInheritancePairs`) from `Map<ConversionPair, V>` to a new internal `ClassPairMap<V>` — a two-tier nested `ClassValueMap<ClassValueMap<V>>` keyed by `(source, target)`. Eliminates per-lookup `ConversionPair` allocation from every `convert()`, `isConversionSupportedFor()`, and `getInheritedConverter()` call; both tiers use `ClassValueMap`'s identity-based fast path. JFR previously showed `ConversionPair` as a top-5 allocation hotspot during json-io reads; gone from the hot path entirely.
...
* **SECURITY**: `ReflectionUtils.isTrustedCaller()` — stack walk no longer misidentifies java-util's own internal cache infrastructure as a trusted caller. Previously matched any frame starting with `com.cedarsoftware.util.`, incorrectly including `LRUCache`, `LockingLRUCacheStrategy`, `ThreadedLRUCacheStrategy`, and `ConcurrentHashMapNullSafe` frames between `ReflectionUtils` and its security check during `computeIfAbsent` callbacks. External callers could therefore bypass dangerous-class blocking on paths routing security through cache-populating lambdas (`getDeclaredFields` shallow, `getMethod`, `getConstructor`). Fix: new `isInfrastructureFrame()` helper skips known passthrough classes by exact name match (no prefix matching).
