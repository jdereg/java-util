...
* **PERFORMANCE**: `RegexUtilities.SafeMatchResult` — replaced eager group extraction with a lazy `MatchResult` snapshot. The constructor previously walked `0..groupCount` and called `matcher.group(i)` on every capturing group, allocating one `String` per group regardless of whether the caller would ever read it. For patterns like `DateUtilities.alphaMonthPattern` (11 groups, of which only 3-4 are typically read on any given match), that's 7-8 wasted `String` substring allocations per call — and the IDE line profiler attributed ~840ms of `JsonPerformanceTest` wall time to the constructor (nearly as much as the regex execution itself at 1,440ms in `matcher.find()`). Fix: hold a `java.util.regex.MatchResult` (via `matcher.toMatchResult()`) plus the input string, and lazy-slice in `group(i)`. `MatchResult` copies only the internal int[] of group start/end indices (cheap), so unaccessed groups now cost zero `String` allocation. Snapshot semantics preserved — the prior contract that callers can hold the result safely after the matcher is reused still holds, since `toMatchResult()` returns an immutable `MatchResult`. All 41 `RegexUtilitiesTest` + 192 `DateUtilitiesTest` + 14 `DateUtilitiesSecurityTest` tests pass; full 20,124-test suite green.
* **PERFORMANCE**: `DateUtilities` — eliminated the regex-call hot path overhead identified in JFR profiling of `safeFind`/`safeMatches` (previously ~2.7s self-time across regex bookkeeping during a `JsonPerformanceTest` run). Two changes: (1) **Cached security configuration**. The five hot getters (`isSecurityEnabled`, `isInputValidationEnabled`, `isMalformedStringProtectionEnabled`, `isRegexTimeoutProtectionEnabled`, `getRegexTimeoutMilliseconds`) previously called `System.getProperty(...)` on every parse — each call grabs the synchronized `Hashtable` lock on the system properties map. Replaced with `volatile` tri-state (`UNRESOLVED`/`FALSE`/`TRUE`) caches resolved on first read and reset to `UNRESOLVED` when properties change between tests. Hot-path cost drops from a synchronized table lookup to a `volatile int` load + branch. New package-private `resetSecurityConfigCacheForTesting()` lets test fixtures invalidate the cache after `System.setProperty(...)` calls. (2) **Per-`Pattern` thread-local `Matcher` reuse**. The fast paths in `safeMatches`/`safeFind` previously called `pattern.matcher(input)` on every invocation, allocating a `Matcher` plus its internal `groups[]`/`locals[]`/`localsPos[]` arrays. Replaced with a `ConcurrentMap<Pattern, ThreadLocal<Matcher>>` cache: each thread keeps one reusable `Matcher` per `Pattern`, reset to the new input on each call. `Matcher.usePattern()` would re-allocate the state arrays when the pattern changes, so per-pattern thread-locals are required. Slow paths (timeout-protected via the `DATE_REGEX_EXECUTOR`) still allocate fresh `Matcher`s — they cross thread boundaries and can't safely share a thread-local. `SafeMatchResult` snapshot semantics preserved (no `Matcher` reference held after construction). All 192 `DateUtilitiesTest` and 14 `DateUtilitiesSecurityTest` tests pass; full 20,124-test java-util suite green.
* **PERFORMANCE**: `MathUtilities` now exposes fast number parsing entry points — `parseDouble`, `parseFloat`, `parseBigDecimal`, `parseBigInteger`, each with `CharSequence`, `char[]`, `char[]+offset+length`, `byte[]`, and `byte[]+offset+length` overloads (20 new public methods). The `char[]` and `byte[]` overloads avoid the intermediate `String` materialization that callers like JSON parsers previously paid before `Double.parseDouble`. Backed by the vendored Randelshofer FastDoubleParser; output is bit-identical to the JDK reference parsers, proven by the 150-test `FastDoubleParserParityTest` (NaN, ±Inf, denormals, hex floats, scientific notation, 100-digit and 500-digit BigIntegers, malformed-input exception parity). Converter rewiring: `StringConversions.toFloat`/`toDouble`/`toBigDecimal`/`toBigInteger`/`toLong`/`toDuration` and `NumberConversions.floatingPointToBigInteger` now route through these entry points; `MathUtilities.parseToMinimalNumericType` likewise uses them internally. Anything calling `Converter.convert(String, Double.class)` (and the analogous Float/BigDecimal/BigInteger paths) picks up the speedup transparently.
* **PERFORMANCE**: Vendored Werner Randelshofer's FastDoubleParser (MIT licensed, https://github.com/wrandelshofer/FastDoubleParser, Java 8 source set) under `com.cedarsoftware.util.fastdoubleparser`. Implements the Eisel-Lemire (2020) algorithm for `Double` / `Float` and recursive-multiplication parsers for `BigDecimal` / `BigInteger`; 2-4x faster than `Double.parseDouble` on typical inputs and 5-50x faster than `new BigInteger(String)` on values with 100+ digits. Mirrors the same vendoring choice Jackson made (jackson-core 2.15+) — keeps java-util dependency-free. License headers preserved on every file. Pruned the locale-aware Configurable* parsers and the JSON-strict Json* variants since the universal `JavaDoubleParser`/`JavaFloatParser` handle every shape we need. Source set: 34 files (~5K LOC). Jar size: 736 KB → 849 KB (+113 KB / +15.4%).
* **CORRECTNESS**: `FastReader.BufferSlice` now has an assertion-checked `release()` lifecycle for borrowed slices. With assertions enabled, callers must consume or copy borrowed data and release the slice before the next `FastReader` read/pushback/close operation; attempts to read again with an outstanding borrowed slice fail fast, catching stale-buffer misuse including local-cached slice references. Production behavior remains unchanged when assertions are disabled. Follow-up `JsonPerformanceTest` JFR comparison (`JsonPerformanceTest_2026_04_25_171755.jfr` vs `...161536.jfr`) showed no measurable guard overhead: overall samples/sec stayed effectively flat (49.83 → 49.83), `FastReader.readLineBorrowed` share was flat/slightly down (1.301% → 1.264%), and no `BufferSlice.release()` / assertion-helper leaves appeared.
* **PERFORMANCE**: `FastReader` now exposes borrowed-slice fast paths, `readUntilBorrowed(BufferSlice, int, char, char)` and `readLineBorrowed(BufferSlice)`, for callers that can consume data directly from the current internal buffer. These APIs preserve the existing delimiter semantics (`readUntil` leaves the delimiter unread; `readLine` consumes the line ending) and return `COPY_REQUIRED` without consuming data when pushback is active or the token crosses a buffer boundary, allowing callers to fall back to the existing copying APIs. JSON/TOON consumers avoid the internal-buffer-to-scratch-buffer copy on common short-string/whole-line cases; the latest `JsonPerformanceTest` JFR comparison showed the old copy paths effectively disappear from read phases (`readUntil(...)` stack samples 101→1 toJava and 76→4 toMaps; `readLine(...)` 83→0 and 78→0), with scan cost moving to the borrowed methods. Added focused `FastReaderTest` coverage for delimiter retention, LF/CRLF line consumption, buffer-boundary fallback, and multiple borrowed slices from the same buffer.

#### 4.101.0 - 2026-04-19
* **FEATURE**: `ClassValueSet.containsClass(Class)` — mirrors `ClassValueMap.getByClass`. Skips the null check and the `o.getClass() != Class.class` guard that `contains(Object)` must perform to reject non-`Class` keys, compiling to a near-direct `ClassValue.get(type)` call. Converted 9 hot-path call sites: `MultiKeyMap` (5 — `LEAF_TYPES` ×3, `SIMPLE_ARRAY_TYPES`, `INTEGRAL_TYPES`, on the per-op flatten/hash/equals paths) and `ClassUtilities.SECURITY_CHECK_CACHE` (4 — `BLOCKED_CLASSES` ×2, `INHERITS_FROM_BLOCKED`, `VERIFIED_SAFE_CLASSES`, all on the first-touch security verification chain). Tightened `MultiKeyMap.SIMPLE_ARRAY_TYPES` field type from `Set<Class<?>>` to `ClassValueSet`. 6 new tests in `ClassValueSetTest` cover null-`Class`, absent, present, null-element mapping, removal, and parity with `contains(Object)`. No correctness changes to `ClassValueSet` itself — `membershipCache` and `unmodifiableView` were already `volatile`.
* **FEATURE**: `ClassValueMap.getByClass(Class)` — new typed fast-path lookup that skips the `instanceof Class` guard in `get(Object)`. The regular `get(Object)` path must check whether the key is a `Class` before routing to the `ClassValue` cache (non-`Class` keys fall through to the `ConcurrentHashMap` backing). `getByClass(Class)` skips that guard entirely, compiling to a near-direct `ClassValue.get(type)` call — a JIT-intrinsified per-`Class` load. Converted 30+ hot-path call sites across `ClassUtilities` (13), `Converter.ClassPairMap` (4), `MultiKeyMap` (3), `TypeUtilities`, `Unsafe`, `CompactMap`, and `CollectionHandling` to use the new method. Field declarations that stored a `ClassValueMap` behind a `Map<Class<?>, V>` type were tightened to `ClassValueMap<V>` so the compiler resolves the typed lookup statically. 7 new unit tests in `ClassValueMapTest` cover null-`Class` lookups, the null-key mapping, hit/miss semantics, removal, explicit-null values, and parity with `get(Object)`.
* **CORRECTNESS**: `ClassValueMap` — marked the lazily-populated `cachedEntrySet`, `cachedValues`, and `cachedUnmodifiableView` fields `volatile`. Without volatile, a thread reading the field via the non-synchronized fast path could observe a partially-constructed view (Java allows stores to be reordered with the publishing store). The race is narrow and has not been observed in practice, but the fix eliminates a dormant JMM bug. No behavior change; no perf impact (these fields are touched only on view-accessor calls, not on the `get` hot path).
* **PERFORMANCE**: `StringUtilities` — restored public `getChars(String, int)` + added convenience `getChars(String)` overload, and applied the SIMD-intrinsic bulk-copy pattern to three more hot paths. The two-arg form lets callers pass a length they already computed (avoiding a redundant `s.length()` call); the one-arg form computes `s.length()` once and delegates. `hashCodeIgnoreCase` was already using the pattern (the 4.99.0 CaseInsensitiveMap perf win depends on it); this commit extends it to `levenshteinDistance`, `damerauLevenshteinDistance`, and `decode(String)` (hex). In both Levenshtein variants the inputs are bulk-copied once into local `char[]` before the O(n·m) nested loop, and loop-invariant chars are hoisted out of the inner loop: `levenshteinDistance` now reads each source char once per outer iter instead of once per inner iter, and `damerauLevenshteinDistance` hoists `srcChar`, `srcPrevChar`, and `tgtChar` — dropping the inner-loop char-reads from up to 6 `charAt` calls (source/target each hit 3× for the cost calc AND the transposition check) to 0 charAt + a few plain array loads per iteration. `decode(String)` (used by `EncryptionUtilities` for SHA/encrypted-blob hex decoding) now reads the pair-wise nibbles from the bulk-copied buffer. Also added private `fillCharArray(CharSequence, char[], int)` helper that uses `String.getChars` on String inputs and falls back to `charAt` loops for other CharSequence types (StringBuilder, CharBuffer, custom wrappers). All 19,951 tests pass; CaseInsensitiveMap perf tests confirm no regression on the existing HashMap-backed 2.99× speedup vs `TreeMap(CASE_INSENSITIVE_ORDER)`. Break-even for the optimization is ~7 chars — below that, `charAt` is tied or wins; the applied call sites all exceed that bound comfortably (Levenshtein typically compares 15+ char strings, hex decode ≥64 chars for SHA-256/512).
* **FEATURE**: New `ReflectionUtils.getDirectClassAnnotation(Class, Class)` — a strict, JDK-semantics companion to the existing `getClassAnnotation`. The existing `getClassAnnotation` has always done a Spring-`AnnotationUtils.findAnnotation`-style full-hierarchy walk (class + all superclasses + all interfaces, regardless of `@Inherited`), which is the right behavior for framework scanning but the wrong behavior for identity-tied annotations like JPA `@Entity`, json-io `@IoNaming`/`@IoTypeName`, or Jackson `@JsonNaming` where the annotation author deliberately chose not to mark their annotation `@Inherited`. The new `getDirectClassAnnotation` matches `Class.getAnnotation(Class)` exactly — respects `@Inherited` on the superclass chain, never walks interfaces. It deliberately does NOT consult `CLASS_ANNOTATION_CACHE` because `Class.reflectionData()` already caches annotation proxies per class internally; layering a `ClassAnnotationCacheKey` allocation + `ConcurrentHashMap` probe on top would be pessimization, and mixing direct-lookup entries with hierarchy-walker entries would be a semantic hazard. Javadoc on both methods now explicitly calls out the divergent semantics and when to pick which. 13 new tests in `ReflectionUtilsDirectAnnotationTest` cover null handling, `@Inherited` superclass behavior, interface non-inheritance, and cache isolation (verifying that repeated direct lookups returning null don't poison the walker's cache for the same key). No existing callers affected — this is purely additive.
* **PERFORMANCE**: `FastReader.readUntil` and `FastReader.readLine` — eliminated redundant `GETFIELD` instructions by caching member fields into locals only where the local is demonstrably read **2+ times within its assignment lifetime** (per bytecode-level review). Specific wins: (1) `pushbackBufferSize` hoisted into `pbSize` for the drain loop (3 source uses: guard, while-check, `\r`-ahead check); (2) `pushbackBuffer` hoisted into `pbBuf` inside the drain guard (2 uses: main scan + `\r`-ahead probe); (3) drain wrapped in `if (pbPos < pbSize)` to skip the wasteful unconditional `pushbackPosition = pbPos;` `PUTFIELD` when the pushback is empty (the common case); (4) `buf`, `position`, `limit` hoisted out of the outer scan-while in both methods — `buf` is never reassigned by `refill()` (refill reads INTO the array but doesn't rebind the reference), so it's cached once per call; `pos` and `lim` refresh only inside the refill branch where they're used twice immediately in the same iteration (`-1`/EOF check + `end = lim` boundary init) rather than relying on cross-iteration loop-back to amortize the store cost; (5) `readUntil`'s in-loop refill moved to the **top** of the scan loop, killing the wasteful unconditional fill that previously fired even when the scan hit `maxLen` instead of buffer-end, and removing the redundant `position = pos;` write at the method bottom (all reachable paths already write position before exit); (6) `readUntil`'s scan-exit `position = scanPos;` hoisted ahead of the delimiter/refill branch split since both paths need it written. Hot-path field-access count in `readUntil` common case (empty pushback, single-chunk scan finds delimiter): **10 → 7 field accesses (−30%)**. Same ~30% reduction applies to `readLine`'s common case; multi-chunk lines save 3 GETFIELDs per additional buffer fill. All 40 `FastReaderTest` tests pass; all 3806 json-io tests pass.
* **PERFORMANCE**: `FastReader.readLine(char[], int, int)` — hoisted `b[scanPos]` into a single explicit local variable per scan-loop iteration instead of relying on JIT CSE to coalesce two array loads. The prior `if (b[scanPos] <= '\r') { char c = b[scanPos]; ... }` pattern had two `CALOAD` bytecodes against the same index; the JIT usually CSEs them, but bytecode-level verification confirmed only one `CALOAD` in the emitted hot-path loop body after the refactor, along with one bounds check instead of two. JFR comparison: `readLine` leaf samples dropped from 208 (2.09% of total) to 170 (1.65%) — a **~22% relative reduction** in the method's self-time, saving roughly 1 second of wall-clock on a 200s `JsonPerformanceTest` run. Outer `c <= '\r'` performance prefilter retained as a nested `if` so printable characters (the vast majority) still need only one comparison before increment.
* **PERFORMANCE**: `Converter` — refactored the four internal conversion caches (`CONVERSION_DB`, `FULL_CONVERSION_CACHE`, `USER_DB`, `cacheInheritancePairs`) from `Map<ConversionPair, V>` to a new internal `ClassPairMap<V>` — a two-tier nested `ClassValueMap<ClassValueMap<V>>` keyed by `(source class, target class)`. This **eliminates per-lookup `ConversionPair` allocation** from every `convert()`, `isConversionSupportedFor()`, and `getInheritedConverter()` call, and leverages `ClassValueMap`'s identity-based fast path at both tiers. JFR previously showed `ConversionPair` as a top-5 allocation hotspot during json-io reads; it is now gone from the hot path entirely.
* **PERFORMANCE**: `Converter.USER_DB` — removed the redundant `instanceId` from its keys. `USER_DB` has always been an instance field (per-`Converter`), so keying by `instanceId` was a legacy artifact from an earlier static design. The `instanceId` parameter is also removed from `getInheritedConverter()` and `addIdentityConversionIfNeeded()`.
* **BUG FIX**: `ReflectionUtils.getAllConstructorsInternal()` — added a deterministic tie-breaker to the constructor sort. `Class.getDeclaredConstructors()` returns constructors in JVM-dependent order that varies across runs, especially under bytecode instrumentation (JaCoCo, AspectJ, etc.). When modifier/param-count ties occurred (e.g., two public single-arg varargs constructors like `foo(String...)` vs `foo(Integer...)`), stable sort preserved non-deterministic input order. Fix: tie-breaker prefers `String` parameters (most widely convertible via `Converter`), unwrapping varargs component types for comparison, then falls back to alphabetical type-name ordering. Resolves intermittent ~30% failure rate in `ClassUtilitiesVarargsArrayStoreTest` under JaCoCo.
* **SECURITY**: `ReflectionUtils.isTrustedCaller()` — stack walk no longer misidentifies java-util's own internal cache infrastructure as a trusted caller. Previously matched any frame starting with `com.cedarsoftware.util.`, which incorrectly included `LRUCache`, `LockingLRUCacheStrategy`, `ThreadedLRUCacheStrategy`, and `ConcurrentHashMapNullSafe` frames appearing between `ReflectionUtils` and its security check during `computeIfAbsent` callbacks. External callers could therefore bypass dangerous-class blocking on paths that routed security through cache-populating lambdas (`getDeclaredFields` shallow, `getMethod`, `getConstructor`). Fix: added `isInfrastructureFrame()` helper that skips known passthrough classes by exact name match (no prefix matching, to avoid accidentally excluding legitimate callers).
...
* **REFACTOR**: Replaced 7 instances of `throw new UncheckedIOException(e)` in `EncryptionUtilities` fast-hash methods (`fastMD5`, `fastSHA1`, `fastSHA256`, `fastSHA384`, `fastSHA512`, `fastSHA3_256`, `fastSHA3_512`) with `ExceptionUtilities.uncheckedThrow(e)`, matching the project's standard unchecked-throw pattern — throws the original `IOException` without wrapping, via a generics cast trick that bypasses checked-exception rules.
* **TESTING**: Added `EncryptionUtilitiesCoverageTest` — 63 tests closing JaCoCo gaps.
* **TESTING**: Added `ReflectionUtilsCoverageTest` — 36 tests closing JaCoCo gaps.
* **TESTING**: Added `UrlUtilitiesCoverageTest` — 41 tests closing JaCoCo gaps.
* **TESTING**: Added `IOUtilitiesCoverageTest` — 32 tests for `close()`, `flush()`, and `transfer()` gaps.
* **TESTING**: Added `DeepEqualsCoverageTest` — 74 tests closing JaCoCo gaps.
* **TESTING**: Added `MultiKeyMapCoverageTest` — 52 tests closing JaCoCo gaps.
* **TESTING**: Added `ClassUtilitiesAdditionalCoverageTest` — 48 tests closing JaCoCo gaps.
* **TESTING**: Added `ReflectionUtilsSecurityTest` — 13 tests in the `com.cedarsoftware.test` package, verifying dangerous-class blocking and trusted-caller semantics from an external caller's perspective.
* **TESTING**: Added `ClassUtilitiesSecurityTest` — 28 tests in the `com.cedarsoftware.test` package, verifying external-caller security semantics for `ClassUtilities` APIs.
