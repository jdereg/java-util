 ...
 * DateUtilities - Strengthened date parsing
  1 Day of week allowed (properly ignored).
  2 First (st), second (nd), third (rd), and fourth (th) ... supported.
  3 The default toString() standard date / time displayed by the JVM is now supported as a parseable format.
  4 Extra whitespace can exist within the date string.
  5 Full time zone support added.
  6 The date (or date time) is expected to be in isolation. Whitespace on either end is fine, however, once the date time is parsed from the string, no other content can be left (prevents accidently parsing dates from dates embedded in text).
