...
#### 4.99.0 (Unreleased)
* **FEATURE**: New `Utf8ByteWriter extends Writer` — writes UTF-8 bytes directly to an OutputStream, bypassing the `OutputStreamWriter` → `StreamEncoder` chain. For ASCII content (99%+ of JSON/TOON), each character is a single `buf[pos++] = (byte) c` store. Handles 2-byte, 3-byte, and 4-byte (surrogate pair) UTF-8 encoding inline. Drop-in replacement for `new FastWriter(new OutputStreamWriter(out, UTF_8))` with zero API changes to consumers. Includes `getLastSnippet()` for error context.
