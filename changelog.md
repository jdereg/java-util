...
* **PERFORMANCE**: `Converter` — removed the 1-slot L1 lookup cache that had been added earlier. A benchmark of `ClassValueMap.get()` at ~2.5 ns/op showed the nested map lookup (~5-6 ns total) is competitive with any L1 approach. The previous L1 slot used three separate `volatile` instance fields; under concurrent access, non-atomic visibility of the three writes could produce stale mixed-state reads (returning a cached `Convert<?>` for the wrong `(source, target)` pair). The nested `ClassPairMap` is both safer (no mixed-state window) and allocates nothing.
...
* **NOTE**: The public `ConversionPair` class and `Converter.pair()` factory methods are **unchanged**. They remain as the user-facing key type for `ConverterOptions.getConverterOverrides()`. Only the **internal** cache storage strategy changed — user code that registers custom conversions continues to work exactly as before.
