...
#### 4.104.0 - (Unreleased)
* **PERFORMANCE**: **Lock-free read fast path for the `ReflectionUtils` caches.** The constructor / sorted-constructor / method / field / field-name / class-annotation / method-annotation caches are populate-once and read indefinitely, but every lookup went through `Map.computeIfAbsent`, whose default `LRUCache` LOCKING strategy acquires the cache's exclusive `ReentrantLock` even on a hit — serializing all concurrent reflection (e.g. multi-threaded deserialization) on a single per-cache lock. Lookups now try the non-blocking `get()` first and fall back to `computeIfAbsent` only on a genuine miss. Behavior is identical (negative lookups are sentinels, never `null`, so `get()==null` unambiguously means "absent"), and the strict-LRU strategy, bounded sizes, and swappable `setXxxCache` hooks are all unchanged. Single-class contended micro-benchmark (12-core): at 8 threads **42 → 141 M ops/s (~3.3×)**; the locked path previously failed to scale past ~42 M ops/s at any thread count (and ping-ponged to 18 M at 2 threads). Single-threaded is also marginally faster (no lock/unlock pair on hits). Mixed real-world reflection loads see a smaller gain — the per-call work dilutes the lock — but the serialization point is removed with no downside.
